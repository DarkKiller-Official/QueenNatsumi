...
${chalk.blue.italic('🏆 Connecting to WhatsApp...▶')}`);
	...
	conn.on('open', async () =>
	{
		...
		var logo = await axios.get(Build.ALIVE_LOGO,
		{
		...
		const buttons = [
		{
			buttonId: BUTTHANDLE + 'chek',
			buttonText:
			{
				displayText: '🔱 VERIFY YOUR BOT 💃'
			},
			type: 1
		},
		{
			buttonId: BUTTHANDLE + 'QUEENMATSUMI',
			buttonText:
			{
				displayText: '💃 BOT FULL CMD 💃'
			},
			type: 1
		},
		{
			buttonId: BUTTHANDLE + 'update',
			buttonText:
			{
				displayText: '🚀 BOT UPDATE 🚀'
			},
			type: 1
		}]
		const buttonMessage = {
	...
	setInterval(async () =>
	{
		...
		while (getGMTh == 19 && getGMTm == 1)
		{
			...
			if (video.includes('http') || video.includes('https'))
			{
				...
				var yt = ytdl(VID,
				{
					filter: format => format.container === 'mp4' && ['1080p', '720p', '480p', '360p', '240p', '144p'].map(() => true)
				});
				yt.pipe(fs.createWriteStream('./' + VID + '.mp4'));
				yt.on('end', async () =>
				{
					return await conn.sendMessage(conn.user.jid, fs.readFileSync('./' + VID + '.mp4'), MessageType.video,
					{
						caption: announce,
			...
			else
			{
				...
				{
					var imagegen = await axios.get(image,
					{
						responseType: 'arraybuffer'
					})
					return await conn.sendMessage(conn.user.jid, Buffer.from(imagegen.data), MessageType.image,
					{
						caption: announce
					})
				}
	...
	conn.on('chat-update', async m =>
	{
		...
		if (Build.WELCOME == 'pp' || Build.WELCOME == 'Pp' || Build.WELCOME == 'PP' || Build.WELCOME == 'pP')
		{
			if (msg.messageStubType === 32 || msg.messageStubType === 28)
			{
				...
				if (gb !== false)
				{
					...
					try
					{
						pp = await conn.getProfilePicture(msg.messageStubParameters[0]);
					}
					catch
					{
						pp = await conn.getProfilePicture();
					}
					await axios.get(pp,
					{
						responseType: 'arraybuffer'
					...
					{
						await conn.sendMessage(msg.key.remoteJid, res.data, MessageType.image,
						{
							caption: gb.message
						});
					});
		...
		else if (Build.WELCOME == 'gif' || Build.WELCOME == 'Gif' || Build.WELCOME == 'GIF' || Build.WELCOME == 'GIf')
		{
			if (msg.messageStubType === 32 || msg.messageStubType === 28)
			{
				...
				if (gb !== false)
				{
					var tn = await axios.get(Build.BYE_GIF,
					{
						responseType: 'arraybuffer'
					})
					await conn.sendMessage(msg.key.remoteJid, Buffer.from(tn.data), MessageType.video,
					{
			...
			else if (msg.messageStubType === 27 || msg.messageStubType === 31)
			{
				...
				if (gb !== false)
				{
					var tn = await axios.get(Build.WELCOME_GIF,
					{
						responseType: 'arraybuffer'
					})
					await conn.sendMessage(msg.key.remoteJid, Buffer.from(tn.data), MessageType.video,
					...
					if (sendMsg)
					{
						...
						*/
						...
						catch (error)
						{
							const buttns = [
							{
								buttonId: BUTTHANDLE + '.joi https://chat.whatsapp.com/IvJxkySd2gd7dF6bRp7u6V',
								buttonText:
								{
									displayText: '📛 SUPPORT GROUP JOIN 📛'
								},
								type: 1
							},
							{
