/* 
Heroku plugin for Natsumi - W4RR10R
...
Natsumi.addCommand({Pnatsumi: 'degis ?(.*)', fromMe: true, desc: Lang.DEGİS_DESC}, (async (message, match) => {
...
Natsumi.addCommand({Pnatsumi: 'restart$', fromMe: true, desc: Lang.RESTART_DESC}, (async (message, match) => {
...
Natsumi.addCommand({Pnatsumi: 'shutdown$', fromMe: true, desc: Lang.SHUTDOWN_DESC}, (async(message, match) => {
...
if (Build.WORKTYPE == 'private') {
    ...
    Natsumi.addCommand({Pnatsumi: 'dyno$', fromMe: true, desc: Lang.DYNO_DESC}, (async (message, match) => {
    ...
    Natsumi.addCommand({Pnatsumi: 'dyno$', fromMe: false, desc: Lang.DYNO_DESC}, (async (message, match) => {
...
Natsumi.addCommand({Pnatsumi: 'setvar ?(.*)', fromMe: true, desc: Lang.SETVAR_DESC}, (async(message, match) => {
    ...
    if (match[1] == 'SEND_READ: true' || match[1] == 'SEND_READ: True' || match[1] == 'SEND_READ: TRUE' || match[1] == 'SEND_READ:True' || match[1] == 'SEND_READ:TRUE' || match[1] == 'SEND_READ:ture' || match[1] == 'SEND_READ: ture' || match[1] == 'SEND_READ:ttue' || match[1] == 'SEND_READ:trie' || match[1] == 'SEND_READ: trie' || match[1] == 'SEND_READ:Trie' || match[1] == 'SEND_READ: Trie') {
        ...
        if (Build.LANG == 'TR' || Build.LANG == 'AZ') {
            ...
            return await heroku.patch(baseURI + '/Build-vars', {
                body: {
                    ['SEND_READ']: 'true'
                }
            });
...
Natsumi.addCommand({Pnatsumi: 'delvar ?(.*)', fromMe: true, desc: Lang.DELVAR_DESC}, (async (message, match) => {
...
Natsumi.addCommand({Pnatsumi: 'getvar ?(.*)', fromMe: true, desc: Lang.GETVAR_DESC}, (async (message, match) => {
