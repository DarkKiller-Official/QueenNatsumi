...
Asena.addCommand({Pnatsumi: 'addlydia$', fromMe: true, desc: Lang.ADDLYDIA_DESC}, (async (message, match) => {
    if (!message.reply_message) return await message.reply(Lang.NEED_REPLY);
    if (Build.COFFEEHOUSE_API_KEY == 'true') return await message.client.sendMessage(message.jid, '```You Must Set an API KEY!```\nhttps://github.com/phaticusthiccy/WhatsAsenaDuplicated/wiki/LYDIA-API-KEY', MessageType.text);

    var unix = Date.now() / 1000 | 0;

    var veriler = await LydiaDB.findAll();
    var lydia = new LydiaAI(Build.COFFEEHOUSE_API_KEY);
...
Asena.addCommand({Pnatsumi: 'rmlydia$', fromMe: true, desc: Lang.RMLYDIA_DESC}, (async (message, match) => {
    if (!message.reply_message) return await message.reply(Lang.NEED_REPLY);
    if (Build.COFFEEHOUSE_API_KEY == 'true') return await message.client.sendMessage(message.jid, '```You Must Set an API KEY!```\nhttps://github.com/phaticusthiccy/WhatsAsenaDuplicated/wiki/LYDIA-API-KEY', MessageType.text);
...
Asena.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {   
    if (Build.COFFEEHOUSE_API_KEY === false) return;
    var unix = Date.now() / 1000 | 0;
