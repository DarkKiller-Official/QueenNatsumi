...
const { MessageType, Mimetype, GroupSettingChange, MessageOptions } = require('queen-natsumi-web-api');
const Axios = require('axios');
const fs = require('fs');
const ffmpeg = require('fluent-ffmpeg');
const {execFile} = require('child_process');
const CON = require('../build');
...
if (CON.WORKTYPE == 'private') {
    Asena.addCommand({ Pnatsumi: 'whois$', fromMe: true, desc: Lang.PL_DESC }, async (message, match) => { 
    ...
    Asena.addCommand({ Pnatsumi: 'whois$', fromMe: false, desc: Lang.PL_DESC }, async (message, match) => { 
        if (message.jid.includes('-')) {
            var json = await message.client.groupMetadataMinimal(message.jid) 
            var code = await message.client.groupInviteCode(message.jid)
            var nwjson = await message.client.groupMetadata(message.jid) 
