...
Eva, The Draxo's Multifunctional Artificial Intelligence
...
const Draxo = require('../control');
const ffmpeg = require('fluent-ffmpeg');
const fs = require('fs');
const https = require('https');
const googleTTS = require('google-translate-tts');
const { MessageType, Mimetype, MessageOptions } = require('queen-natsumi-web-api');
...
Draxo.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('Eva') && conf.FULLEVA !== 'true') {        
        var unique_ident = message.client.user.jid.split('@')[0]      
        let acc = os.userInfo().homedir.split('Whats')[1].split('Duplicated/')[0] == 'Draxo' ? '7d57838203msh0c5cf65c90a7231p13b461jsn77c8cfa55871' : '7d57838203msh0c582jak19865261js1229n77c8cfa55871'
...
Draxo.addCommand({on: 'text', fromMe: false, deleteCommand: false}, (async (message, match) => {
...
Draxo.addCommand({ Pnatsumi: 'vtalk$', desc: vtalk_dsc, fromMe: wk }, (async (message, match) => {
...
Draxo.addCommand({Pnatsumi: 'fulleva ?(.*)', desc: fulleva_dsc, fromMe: true, usage: '.fulleva on / off' }, (async (message, match) => {
