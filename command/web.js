...
/*
var LANG = { RES: '' }
if (Build.LANG == 'TR') LANG.RES == '*Komut Bulunamadı!*\n*Benzer Komut:*'
if (Build.LANG == 'EN') LANG.RES == '*Command not Found!*\n*Similar Command:*'
if (Build.LANG == 'AZ') LANG.RES == '*Komanda tapılmadı!*\n*Oxşar əmr:*'
if (Build.LANG == 'PT') LANG.RES == '*Comando não encontrado!*\n*Comando Similar:*'
if (Build.LANG == 'RU') LANG.RES == '*Команда не найдена!*\n*Аналогичная команда:*'
if (Build.LANG == 'ID') LANG.RES == '*Perintah tidak ditemukan!*\n*Perintah serupa:*'
if (Build.LANG == 'ES') LANG.RES == '*¡Comando no encontrado!*\n*Comando similar:*'
if (Build.LANG == 'ML') LANG.RES == '*കമാൻഡ് കണ്ടില്ല!*\n*സമാന കമാൻഡ്:*'
if (Build.LANG == 'HI') LANG.RES == '*यह कमांड नहीं मिला!*\n*समान कमांड:*'
...
Asena.addCommand({Pnatsumi: '?(.*)', fromMe: true, deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
    var HANDLER = '';
...
Asena.addCommand({Pnatsumi: 'speedtest', fromMe: true, desc: Lang.SPEEDTEST_DESC}, (async (message, match) => {
...
*/
Asena.addCommand({Pnatsumi: 'ping$', fromMe: true, deleteCommand: false, desc: Lang.PING_DESC}, (async (message, match) => {
...
if (Build.WORKTYPE == 'private') {
    ...
    Asena.addCommand({Pnatsumi: 'short ?(.*)', fromMe: true, desc: Lang.URL}, (async (message, match) => {
    ...
    Asena.addCommand({Pnatsumi: 'calc ?(.*)', fromMe: true, desc: Lang.CALC }, (async (message, match) => {
    ...
    Asena.addCommand({Pnatsumi: 'short ?(.*)', fromMe: false, desc: Lang.URL}, (async (message, match) => {
    ...
    Asena.addCommand({Pnatsumi: 'calc ?(.*)', fromMe: false, desc: Lang.CALC }, (async (message, match) => {
        if (match[1].length < 4) { return await message.client.sendMessage(message.jid,Lang.VALİD, MessageType.text) }
        if (match[1].includes('+')) { var split = match[1].split('+'), sonsayi = split[1], ilksayi = split[0]
            var result = -(-ilksayi - sonsayi)
            try { await message.client.sendMessage(message.jid,Lang.SUC + result, MessageType.text) }
