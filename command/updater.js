...
Asena.addCommand({Pnatsumi: 'update$', fromMe: true, desc: Lang.UPDATER_DESC}, (async (message, match) => {
    await git.fetch();
...
Asena.addCommand({Pnatsumi: 'update now$', fromMe: true, desc: Lang.UPDATE_NOW_DESC}, (async (message, match) => {
    await git.fetch();
    var commits = await git.log([Build.BRANCH + '..origin/' + Build.BRANCH]);
    if (commits.total === 0) {
